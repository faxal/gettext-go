...
func TestCallerName(t *testing.T) {
	...
	// init
	name = `github.com/faxal/gettext-go/gettext.init`
	...
	// tInit -> gettext.func
	name = `github.com/faxal/gettext-go/gettext.func`
	if s := tCaller(0); s != name {
		t.Fatalf("expect = %s, got = %s", name, s)
	}

	// caller stack
	name = `github.com/faxal/gettext-go/gettext.callerName`
	if s := callerName(0); s != name {
		t.Fatalf("expect = %s, got = %s", name, s)
	}
	name = `github.com/faxal/gettext-go/gettext.TestCallerName`
